...
final class QRCodeEncoder {
	...
	private boolean encodeContentsFromZXingIntent(Intent intent) {
		...
		String formatString = intent.getStringExtra(Const.Encode.FORMAT);
		try {
		...
		if (format == null || BarcodeFormat.QR_CODE.equals(format)) {
			String type = intent.getStringExtra(Const.Encode.TYPE);
			...
			String data = intent.getStringExtra(Const.Encode.DATA);
	...
	private void encodeQRCodeContents(Intent intent, String type) {
		if (type.equals(Const.Type.TEXT)) {
			String data = intent.getStringExtra(Const.Encode.DATA);
			if (data != null && data.length() > 0) {
				contents = data;
				displayContents = data;
		...
		} else if (type.equals(Const.Type.EMAIL)) {
			String data = trim(intent.getStringExtra(Const.Encode.DATA));
			if (data != null) {
				contents = "mailto:" + data;
				displayContents = data;
				title = "Email address";
			}
		} else if (type.equals(Const.Type.PHONE)) {
			String data = trim(intent.getStringExtra(Const.Encode.DATA));
			if (data != null) {
				contents = "tel:" + data;
		...
		} else if (type.equals(Const.Type.SMS)) {
			String data = trim(intent.getStringExtra(Const.Encode.DATA));
			if (data != null) {
				contents = "sms:" + data;
				displayContents = PhoneNumberUtils.formatNumber(data);
		...
		} else if (type.equals(Const.Type.CONTACT)) {
			Bundle bundle = intent.getBundleExtra(Const.Encode.DATA);
			if (bundle != null) {
				...
				for (int x = 0; x < Const.PHONE_KEYS.length; x++) {
					String phone = trim(bundle.getString(Const.PHONE_KEYS[x]));
					if (phone != null) {
				...
				for (int x = 0; x < Const.EMAIL_KEYS.length; x++) {
					String email = trim(bundle.getString(Const.EMAIL_KEYS[x]));
					if (email != null) {
		...
		} else if (type.equals(Const.Type.LOCATION)) {
			Bundle bundle = intent.getBundleExtra(Const.Encode.DATA);
			if (bundle != null) {
